...
EGLUTWindow::EGLUTWindow(const std::string& title, int width, int height, GraphicsApi api) : WindowWithLinuxJoystick(title, width, height, api), title(title), width(width), height(height), graphicsApi(api) {
...
void EGLUTWindow::_eglutMouseButtonFunc(int x, int y, int btn, int action) {
    ...
    currentWindow->onMouseButton(x, y, btn, action == EGLUT_MOUSE_PRESS ? MouseButtonAction::PRESS : MouseButtonAction::RELEASE);
}
...
void EGLUTWindow::_eglutKeyboardSpecialFunc(int key, int action, unsigned int meta) {
    if(currentWindow == nullptr)
        return;
    int mods = translateMeta(meta);
    KeyCode mKey = getKeyMinecraft(key);
    KeyAction enumAction = (action == EGLUT_KEY_PRESS ? KeyAction::PRESS : (action == EGLUT_KEY_REPEAT ? KeyAction::REPEAT : KeyAction::RELEASE));
    if(mKey != KeyCode::UNKNOWN) {
        currentWindow->onKeyboard(mKey, enumAction, mods);
    }
    if((key == 86 || key == 118) && mods & KEY_MOD_CTRL && action == EGLUT_KEY_PRESS) {
        eglutRequestPaste();
    }
#ifndef NDEBUG
...
int EGLUTWindow::translateMeta(unsigned int meta) {
    int mods = 0;
    if(meta & ShiftMask) {
        mods |= KEY_MOD_SHIFT;
    }
